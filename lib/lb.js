...
const activeGreenLog = "ACTIVE_GREEN";
const activeBlueLog = "ACTIVE_BLUE";
...
class Production
{
    ...
    proxy()
    {
      try {
        ...
        let server  = http.createServer(function(req, res)
        {
            ...
            // Add log entry
            let logEntry = Date.now() + " " + that.TARGET + "\n";
            fs.writeFile(accessLogFile, logEntry, { flag: 'a' }, err => {
              if (err) {
                console.error(err);
              }
            });
   ...
   async updateServers()
   {
        ...
        for (let i=0; i<this.poolSize; i++) {
          let url = "http://" + this.properties.get("GREEN_IP_" + i) + ":" + this.port;
        ...
        for (let i=0; i<this.poolSize; i++) {
          let url = "http://" + this.properties.get("BLUE_IP_" + i) + ":" + this.port;
          await isActive(url).then(function(op) {
            if (op) {
        ...
        writeArray(this.ACTIVE_GREEN, activeGreenLog);
        writeArray(this.ACTIVE_BLUE, activeBlueLog);
   }
}
...
async function writeArray(array, fileName) {
