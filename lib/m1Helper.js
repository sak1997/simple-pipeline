...
class M1Helper {
  ...
  constructor() {
    this.sshConfig = {
        ...
        port: 22,
        user: 'ubuntu',
        identifyFile: "~/Library/Application Support/basicvm/key"
    };
  }
  ...
  async init() {
    ...
    // }
    await execCmd(`vm stop ${name} >> /dev/null`);
    await execCmd(`vm rm ${name} >> /dev/null`);
  ...
  async moveToDeployEnv() {
    await sshExec("rm -rf /home/ubuntu/ssh", this.sshConfig);
    await sshExec("'cp -r /home/ubuntu/shared/cwd/ssh /home/ubuntu/'", this.sshConfig);
  }
